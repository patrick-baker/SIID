...
class TotalBias extends Component {
    ...
    render() {
        return (
            <div ref="chart" className="report__totalBias__all">
                {/*making sure this number and the none value percentages add to 100 even though there would be some rounding*/}
                <p className="report__totalBias__percent">{100 - (Math.floor((this.state.total - this.state.bias) / this.state.total * 100))}%</p>
                <div className="report__totalBias__text">
                    <p>of sentences contain sensitive sentiments</p>
                </div>
            </div>
        );
    }
